...
public class border_select2 : MonoBehaviour
{
    ...
    void MoveObject(int data)
    {
        ...
        if (data == 4 && !jsLeft && Char_Select.Player2 == -1) //up
        {
            jsLeft = true;
            sound.GetComponent<AudioSource>().Play();
            if (targetPos == 0)
        ...
        else if (data == 5 && !jsRight && Char_Select.Player2 == -1) //down
        {
            jsRight = true;
            sound.GetComponent<AudioSource>().Play();
            if (targetPos == selectPos.Length - 1)
