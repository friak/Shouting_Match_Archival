...
public class player_1_controls : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (frameState == "wind up1")
        {
            if (timeOut >= 0)
            {
                spriteRenderer.sprite = Player_Windup01;
                timeOut -= Time.deltaTime; // shorthand for: timeOut = timeOut - Time.deltaTime
            }
            else
            { 
                    GameObject atk = Instantiate(Light, shotPoint.position, shotPoint.rotation);
                    atk.GetComponent<attack_script>().enemy_hp = enemy_hp;
                    atk.GetComponent<attack_script>().player = "player1";
                    frameState = "attack";
                    timeOut = .25f;
            }
        }
        ...
        if (frameState == "wind up2")
        {
            if (timeOut >= 0)
            {
                spriteRenderer.sprite = Player_Windup02;
                timeOut -= Time.deltaTime; // shorthand for: timeOut = timeOut - Time.deltaTime
            }
            else
            {
                GameObject atk = Instantiate(Mid, shotPoint.position, shotPoint.rotation);
                atk.GetComponent<attack_script>().enemy_hp = enemy_hp;
                atk.GetComponent<attack_script>().player = "player1";
                frameState = "attack";
                timeOut = .25f;
        ...
        if (frameState == "wind up3")
        {
            if (timeOut >= 0)
            {
                spriteRenderer.sprite = Player_Windup03;
                timeOut -= Time.deltaTime; // shorthand for: timeOut = timeOut - Time.deltaTime
            }
            else
            {
        ...
        if (Input.GetKeyDown(KeyCode.W) && frameState == "idle")
        {
            targetPos += distance;
            if (targetPos >= 1.5f)
            {
        ...
        else if (Input.GetKeyDown(KeyCode.S) && frameState == "idle")
        {
            targetPos -= distance;
            if (targetPos <= -3f)
            {
        ...
        else if (Input.GetKeyDown(KeyCode.A)  && frameState == "idle")
        {
            frameState = "block";
        }
        else if (Input.GetKeyUp(KeyCode.A) && frameState == "block")
        {
            frameState = "idle";
        }
        if (frameState == "idle")
